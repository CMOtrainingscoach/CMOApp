...
export async function LabReviewPage(opts: {
  ...
  const { data: review } = await admin
    ...
    .select(
      "score, strengths, weaknesses, required_revisions, verdict, feedback_md",
    )
            ...
            <div className="space-y-6">
              ...
              <div className="card-premium p-6 flex items-center justify-between gap-4 flex-wrap">
                {passed ? (
                  ...
                  <>
                    <p className="text-sm text-text-secondary max-w-md">
                      Address the required revisions and resubmit. The Professor
                      wants sharper, not more.
                    </p>
                    <Link
                      href={`${modHub}/assignment`}
                      className="btn-gold px-5 py-2.5 inline-flex items-center gap-2"
                    >
