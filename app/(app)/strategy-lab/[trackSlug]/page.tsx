...
import { ArrowLeft, ArrowRight, CheckCircle2, Lock, Trophy } from "lucide-react";
...
export default async function TrackPage({
  ...
  const { data: track } = await admin
    ...
    .select("id, slug, title, tagline, description, is_active")
    .eq("slug", trackSlug)
    .maybeSingle();
  if (!track) notFound();
  ...
  // Determine module unlocks (concurrently)
  ...
  // Get latest assignment review verdict per module (for "module passed" indicator)
        ...
        {!track.is_active && (
          <div className="card-premium p-8 max-w-2xl">
            ...
            <p className="text-sm text-text-secondary leading-relaxed">
              ...
              <Link
                href="/strategy-lab/positioning-strategy"
                className="text-gold-300 underline underline-offset-4"
              >
                Positioning Strategy
              </Link>{" "}
              to build the foundational executive muscle.
            </p>
          </div>
        )}
...
function ModuleCard({
  ...
  lessonsTotal: number;
  lessons: { id: string; ord: number; title: string; completed: boolean; minutes: number }[];
}) {
  return (
    <div
      ...
      {unlocked && (
        <ul className="mt-5 space-y-1.5">
          {lessons.map((l) => (
            <li key={l.id}>
              <Link
                href={`/strategy-lab/${trackSlug}/${moduleId}/${l.id}`}
