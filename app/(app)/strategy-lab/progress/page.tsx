...
import { getUserLevel, RANKS, XP_AMOUNTS } from "@/lib/strategy/xp";
...
export default async function ProgressPage() {
  ...
  const [{ data: profile }, level, { data: events }, { data: rewards }] =
    await Promise.all([
      ...
      admin
        .from("reward_unlocks")
        .select("id, unlocked_at, viewed_at, module_rewards:reward_id(title, kind, module_id)")
        .eq("user_id", user.id)
  ...
  return (
    <>
      <Topbar
        ...
        subtitle="Your XP, your rank, your trail of work."
      />
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="card-premium p-6">
            ...
            <p className="text-text-muted text-sm mt-1">
              Longest: {level.longest_streak} days
            </p>
          </div>
        ...
        <section>
          ...
          {(events ?? []).length === 0 ? (
            <div className="card-premium p-8 text-center text-text-muted">
              Complete a lesson or pass an assignment to begin logging XP.
            </div>
          ) : (
