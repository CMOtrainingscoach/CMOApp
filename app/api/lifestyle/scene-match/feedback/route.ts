...
import { LIFESTYLE_SCENE_MATCH_FEEDBACK_SYSTEM } from "@/lib/prompts";
...
export async function POST(req: Request) {
  ...
  if (parsed.misses.length > 0) {
    missLines = parsed.misses
      .map(
        (m, i) =>
          `${i + 1}. Person: "${m.term}"\n   Known for (correct): "${m.expectedDef}"\n   They matched: "${m.choseDef}"`,
      )
      .join("\n\n");
  }
  ...
  const userBlock = [
    ...
    parsed.misses.length === 0 ?
      "Result: PERFECT ROUND — acknowledge briefly and deepen one recognition habit."
  ...
  try {
    const { text } = await generateText({
      ...
      system: LIFESTYLE_SCENE_MATCH_FEEDBACK_SYSTEM,
      prompt: userBlock,
      temperature: 0.55,
      maxTokens: 520,
    });
    ...
    console.error("[lifestyle scene-match feedback]", e);
